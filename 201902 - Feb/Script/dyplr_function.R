...
Employee_Diversity_in_Tech <- read_delim("GitHub/RLadies/meetup-presentations_bari/2019 - Feb/dataset/Employee Diversity in Tech.csv", 
...
select(Employee_Diversity_in_Tech, data = Date, Tipo = Type, Azienda = Company)
...
head(select(Employee_Diversity_in_Tech, Sepal.Length:Petal.Length))
...
# To filter the rows for Sepal.Length >= 4.6, you can use:
  filter(Employee_Diversity_in_Tech, Sepal.Length >= 4.6)

# To filter the rows for Sepal.Length >= 4.6 and Petal.Width >= 0.5, you can use:
  filter(Employee_Diversity_in_Tech, Sepal.Length >= 4.6, Petal.Width>=0.5)
...
?group_by
  ...
  # 
  # The group_by() verb is an important function in dplyr. We want to split the data frame by some variable (e.g. Sepal.Length), apply a function to the individual data frames, and then combine the output.
...
# The mutate() function will add new columns to the data frame. Create a new column called proportion, which is the ratio of Sepal.Length to Sepal.Width.
iris %>%
...
# 
# The summarize() function will create summary statistics for a given column in the data frame. For example, to compute the average number of Sepal.Length, apply the mean() function to the column Sepal.Length and call the summary value "avg_slength".
iris %>%
